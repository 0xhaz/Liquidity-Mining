...
import {PoolExtended} from "./libraries/PositionExtended.sol";
...
contract LiquidityMiningHook is BaseHook {
    ...
    mapping(PoolId => BalanceDelta simulatedSwap) public swapCache;
    ...
    /// @param rate the rate at which to stream the token
    ...
    /// @notice Allows the stream creator to kill the stream and withdraw any unstreamed tokens so far
    /// @param id the pool id
    /// @param tickLower the lower tick of the range
    /// @param tickUpper the upper tick of the range
    /// @param streamToken the token to stream
        ...
        returns (bytes4, BeforeSwapDelta, uint24)
    {
        ...
        // we want to check that the simuated swap must equal to the actual swap
    ...
    function afterSwap(
        ...
        // store the simulated swap delta to check against it in the afterSwap hook
    ...
    /// @param streamToken The stream token
    /// @param rate The per second stream rate of the token
    ...
    /// @return unclaimedStreams The unclaimed streams of the position
